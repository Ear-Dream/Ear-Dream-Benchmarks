...
def main():
    ...
    if len(selected) != 3000:
        raise ValueError(f"Expected 3000 selected words, found {len(selected)}")
    word_ids = [int(r["단어 번호"]) for r in selected]
    if len(set(word_ids)) != 3000:
        raise ValueError("Selected word IDs are not unique")
    classes = {f"WORD{word_id:04d}": i for i, word_id in enumerate(word_ids)}
    names = {str(i): {"word_id": word_ids[i], "word": selected[i]["단어"]} for i in range(3000)}
    name_by_id = {word_ids[i]: selected[i]["단어"] for i in range(3000)}
    ...
    for row in source_rows:
        ...
        chosen.append({
            ...
            "word_id": word_id, "word": name_by_id[word_id],
