...
def main():
    ...
    class_count=len(json.loads((manifest.parent/"classes.json").read_text(encoding="utf-8")))
    ...
    missing = {split: sorted(set(range(class_count)) - set(counts)) for split, counts in split_classes.items()}
