...
def main():
    ...
    model=make_model(cfg,num_classes,dropout).to(device)
    ...
    with history_path.open("a",encoding="utf-8",newline="") as stream:
        ...
        for epoch in range(start,epochs):
            model.train(); seen=correct=0; loss_sum=0.0; tick=time.time(); stepped_any=False
            for batch in train_loader:
                ...
                scaler.scale(loss).backward(); scaler.unscale_(optim); nn.utils.clip_grad_norm_(model.parameters(),cfg["training"]["gradient_clip_norm"])
                scale_before=scaler.get_scale(); scaler.step(optim); scaler.update(); stepped_any = stepped_any or scaler.get_scale() >= scale_before
